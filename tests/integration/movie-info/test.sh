...
testMovieInfo(){
  ...
  result=$(dc-movie-info -s tests/integration/movie-info/movie.m4v | jq -r -c .)
  local exit=$?

  dc-tools::assert::equal "$exit" "0"
  dc-tools::assert::equal "$result" "$expected"

  expected='{"file":"tests/integration/movie-info/movie.mkv","size":"11402","container":"matroska,webm","description":"Matroska / WebM","fast":"false","duration":"1","video":[{"id":0,"codec":"h264","description":"H.264 / AVC / MPEG-4 AVC / MPEG-4 part 10","width":558,"height":418}],"audio":[{"id":1,"codec":"aac","description":"AAC (Advanced Audio Coding)","language":"eng"}],"subtitles":[],"other":[]}'
  result=$(dc-movie-info -s tests/integration/movie-info/movie.mkv | jq -r -c .)
  local exit=$?
  ...
  result=$(dc-movie-info -s tests/integration/movie-info/movie.mov | jq -r -c .)
  local exit=$?
  ...
  result=$(dc-movie-info -s tests/integration/movie-info/movie.avi | jq -r -c .)
  local exit=$?
